...
{
    private readonly int Default = 4;
    private readonly Dictionary<Stat, StatData> ActualStat;

    public RPGStats()
    {
        ActualStat = new Dictionary<Stat, StatData>(8);
        for (var i = 0; i <= 7; i++) ActualStat.Add((Stat)i, new StatData(Default));
    }
    ...
    {
        for (var i = 0; i <= 7; i++) ActualStat[(Stat)i] = new StatData(level + Default - 1);
    }

    public void OnLevelUp()
    {
        for (var i = 0; i <= 7; i++) ActualStat[(Stat)i].LevelUp();
    }
}