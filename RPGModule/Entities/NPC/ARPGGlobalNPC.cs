...
internal class ARPGGlobalNPC : GlobalNPC
{
    private int baseDamage;
    public float debuffDamage;
    public bool DontCreateStats = false;
    public float dontTakeDamageTime;
    public NPCModifier modifier = 0;

    private NPCRank Rank = NPCRank.Normal;
    public Dictionary<string, string> specialBuffer = new();
    public bool StatsCreated;
    public bool StatsFrame0;

    private int updateDataFrame;

    private float VauraMemory;
    public int getLevel { get; private set; } = -1;

    public int getTier { get; private set; }
    ...
    {
        npc.life = (npc.life - damage).Clamp(0, npc.lifeMax);
    ...
    public int ApplyVampricAura(NPC npc, float damage)
    {
        ...
        VauraMemory -= value;
        npc.life = (npc.life - value).Clamp(0, npc.lifeMax);
        Dust.NewDust(npc.Center - npc.Size * 0.5f, npc.width, npc.height, 90, 0f, 0f, 0, new Color(255, 50, 0), 0.1f);
    ...
    {
        npc = NPCUtils.SetNPCStats(npc, getLevel, getTier, Rank);
    ...
    public void SetInit(NPC npc)
    {
        if (Main.netMode == NetmodeID.Server)
            Main.npcLifeBytes[npc.type] =
                4; //Sadly have to UN-optimise Health of ennemy, because it caused npc to dispear if health was over 128 (for small )
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient)
            if (getLevel < 0)
            {
                ...
                {
                    Rank = NPCUtils.GetRank(getLevel + getTier, npc.boss);
                    modifier = NPCUtils.GetModifier(Rank, npc);

                    if (HaveModifier(NPCModifier.Size))
                    {
                        var maxrng = Main.expertMode ? 50 : 70;
                        var rn = Mathf.RandomInt(0, maxrng);
    ...
    private void Effect(NPC npc)
    {
        ...
        {
            Dust.NewDust(npc.Center - npc.Size * 0.5f, npc.width, npc.height, 244, 0f, 0f, 0, new Color(255, 255, 255),
                0.05f);
        ...
        {
            Dust.NewDust(npc.Center - npc.Size * 0.5f, npc.width, npc.height, 90, 0f, 0f, 0, new Color(255, 255, 255),
                0.5f);
        ...
        {
            Dust.NewDust(npc.Center - npc.Size * 0.5f, npc.width, npc.height, 91, 0f, 0f, 0, new Color(255, 255, 255),
                0.5f);
        ...
        {
            var num41 = Dust.NewDust(npc.position, npc.width, npc.height, DustID.Poisoned, 0f, 0f, 120, default, 0.2f);
    ...
    public override void PostAI(NPC npc)
    {
        ...
        if (!StatsCreated && Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            MPPacketHandler.SendNpcSpawn(Mod, -1, npc, getTier, getLevel, this);
            //NetMessage.SendData(23, -1, -1, null, npc.whoAmI);
            npc.GivenName = NPCUtils.GetNpcNameChange(npc, getTier, getLevel, Rank);
        }
        ...
        if (Main.netMode == NetmodeID.MultiplayerClient)
        {
            ...
            else if (!StatsCreated) MPPacketHandler.AskNpcInfo(Mod, npc, Main.myPlayer);
        }
        ...
        if (debuffDamage > 0)
        {
            var applydamage = Mathf.FloorInt(debuffDamage);
        ...
        if (HaveModifier(NPCModifier.Berserker))
            npc.damage = Mathf.RoundInt(baseDamage * (2 - npc.life / (float)npc.lifeMax));
    ...
    public override void OnKill(NPC npc)
    {
        ...
        {
            var clusterRN = Mathf.RandomInt(4, 8);
            for (var i = 0; i < clusterRN; i++)

                NPCUtils.SpawnSized(npc, Entity.GetSource_NaturalSpawn(), specialBuffer);
        }

        var XPToDrop = NPCUtils.GetExp(npc);
        if (npc.rarity > 0) XPToDrop = (int)(XPToDrop * 1.5f);
        ...
        else if (Main.netMode == NetmodeID.Server) MPPacketHandler.SendXPPacket(Mod, XPToDrop, xplevel);
    ...
    {
        var damage = modifiers.FinalDamage.Additive * modifiers.FinalDamage.Multiplicative;

        if (HaveModifier(NPCModifier.Vampire)) npc.HealEffect(Mathf.RoundInt(damage * 0.5f));
